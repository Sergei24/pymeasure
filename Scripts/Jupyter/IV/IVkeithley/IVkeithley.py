...
class IV(Procedure):
    ...
    current_range = FloatParameter('Maximum Current', units='nA', default=100)
    ...
    def startup(self):
        ...
        self.meter = Keithley2182("GPIB::5")
        self.meter.setup()

        self.source = Keithley6220("GPIB::12")
        self.source.enable_auto_range()
        self.source.source_current_range = self.current_range*1e-9  # calculates in Amperes
        self.source.complinance_voltage = 1  # Volts
        self.source.reset()
        self.source.enable_source()
        sleep(2)

    def execute(self):
        currents = CurrentValues(current_range, current_step)
        currents *= 1e-9  # and finally change to A
        steps = len(currents)
